...
export default function UserProfilePage() {
  ...
  return (
    <Layout title={isPublic ? profile.displayName : (profile.username ?? "Profile")}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.09 }}
          ...
          {earnedList.length === 0 ? (
            ...
            <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {earnedList.map(a => (
                <div key={a.key}
                  className="flex items-center gap-3 p-3 rounded-2xl bg-yellow-500/5 border border-yellow-500/15">
